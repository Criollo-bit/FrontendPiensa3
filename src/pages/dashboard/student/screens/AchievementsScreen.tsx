...
const ICON_MAP: { [key: string]: string } = {
  'trophy': trophy, 'star': star, 'flash': flash, 'flame': flame,
  'book': book, 'shield-checkmark': shieldCheckmark, 'bulb': bulb, 'footsteps': videocam, 
};

interface Achievement {
  id: number; name: string; description: string; icon: string;
  pointsReward: number; unlocked: boolean; unlockedAt?: string; unlockCondition?: string;
}
...
const AchievementsScreen: React.FC<AchievementsScreenProps> = ({ user }) => {
  ...
  const totalPoints = achievements.filter(a => a.unlocked).reduce((sum, a) => sum + a.pointsReward, 0);
  ...
  // 🔥 NOTA: Cambiamos IonPage/IonContent por un div contenedor
  return (
    <div className="achievements-container">
           ...
           <div className="achievements-grid">
             {achievements.map((ach) => (
               ...
               >
                 ...
                 {ach.unlocked ? (
                   <div className="status-badge unlocked"><IonIcon icon={checkmarkCircle} /></div>
                 ) : (
                   <div className="status-overlay locked"><IonIcon icon={lockClosed} /></div>
                 )}
               </div>
             ))}
           </div>
       ...
       {/* Espaciador inferior para navbar */}
             ...
             <div className="modal-body">
               {selectedAchievement.unlocked ? (
                 ...
                 <div className="locked-content">
                   <div className="lock-circle"><IonIcon icon={lockClosed} /></div>
